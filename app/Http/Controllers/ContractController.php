        ...
        try {
            DB::transaction(function () use ($request) {
                ...
                $ugovor = Contract::create([
                    'zaduzen_za_implementaciju' => $request->worker['id'],
                    'firma_id' => $company->id,
                ...
                foreach ($request->functionalities as $fc) {
                    if (isset($fc['id'])) {
                        $ugovor->functionalities()->attach($fc['id'],
                            [
                                'uradjeno' => $fc['isDone'] ?? 0,
                                'izabrano' => $fc['selected'] ?? false
                            ]);
                    }
                }
                ...
                if ($contract->company) {
                    ...
                    foreach ($request->functionalities as $fc) {
                        if (isset($fc['id'])) {
                            ...
                            $syncData[$fc['id']] = ['uradjeno' => $fc['pivot']['uradjeno'], 'izabrano' => $fc['pivot']['izabrano']];
                        }
                    }
                    foreach($request->customFunctionalities as $custom) {
                        // For new functionalities
                        $func = Functionalities::create([
                            'trziste_id' => $contract->company->trziste_id,
                            'funkcionalnost' => $fc['funkcionalnost'],
                            'type' => 'custom'
                        ]);
                        $syncData[$func->id] = ['uradjeno' => $fc['pivot']['uradjeno'], 'izabrano' => $fc['pivot']['izabrano']];
                    }

                    $contract->functionalities()->sync($syncData);
                    //Contract
                    $contract->update([
                        'zaduzen_za_implementaciju' => $request->worker,
                        'fakturisanje_id' => $request->tip_fakturisanja,
                        'broj_aneksa' => $request->aneks,
                        'status_id' => $request->implementation_status,
                        'paket_id' => $request->package,
